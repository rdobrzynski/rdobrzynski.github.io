...
```{r}
d2 <-df%>% filter(Province_State==c('Massachusetts','Rhode Island','Vermont','New Hampshire','Maine'))%>% group_by(Date, Vaccine_Type) %>% summarise(vac_admin=mean(Doses_admin, na.rm=TRUE))
...
```{r}
df%>% filter(Province_State==c('Massachusetts','Rhode Island','Vermont','New Hampshire','Maine')) %>% ggplot()+geom_line(mapping=aes(x=Date,y=Doses_admin,color=Province_State))
```

- Use `geom_line` to compare states in New England in term of the total vaccine administered by month. 
```{r}
df$months <- month(df$Date)
df%>% filter(Province_State==c('Massachusetts','Rhode Island','Vermont','New Hampshire','Maine')) %>% ggplot()+geom_line(mapping=aes(x=months,y=Doses_admin,color=Province_State))
```
